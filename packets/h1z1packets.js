...
function readPacketType(data, packets) {
    let opCode = data[0] >>> 0, length = 0, packet;
    if (packets[opCode]) {
...
function packPositionUpdateData(obj) {
    let data = new Buffer.alloc(7), flags = 0, v;
...
function parseVehicleReferenceData(data, offset) {
    ...
    const inSize = data.readUInt32LE(0), outSize = data.readUInt32LE(4), compData = data.slice(8);
    data = utils_1.lz4_decompress(compData, inSize, outSize);
    const result = h1z1_dataschema_1.default.parse(vehicleReferenceDataSchema, data, 0).result;
    return {
        value: result,
        length: dataLength + 4,
    };
...
function packVehicleReferenceData(obj) {
    const data = h1z1_dataschema_1.default.pack(vehicleReferenceDataSchema, obj);
    return data;
}
function parseItemAddData(data, offset, referenceData) {
    const itemDataLength = data.readUInt32LE(offset);
    offset += 4;
    let itemData = data.slice(offset, offset + itemDataLength);
    const inSize = itemData.readUInt16LE(0), outSize = itemData.readUInt16LE(2), compData = itemData.slice(4, 4 + inSize), decompData = utils_1.lz4_decompress(compData, inSize, outSize), itemDefinition = h1z1_dataschema_1.default.parse(baseItemDefinitionSchema, decompData, 0).result;
...
const effectTagsSchema = [
    ...
    { name: "unknownQword5", type: "uint64", defaultValue: "0" },
    ...
    { name: "unknownQword6", type: "uint64", defaultValue: "0" },
    { name: "unknownQword7", type: "uint64", defaultValue: "0" },
...
const [weaponPacketTypes, weaponPacketDescriptors] = packettable_1.default(weaponPackets);
function parseMultiWeaponPacket(data, offset) {
    const startOffset = offset, packets = [];
...
function parseWeaponPacket(data, offset) {
    ...
    if (weaponPacket.packet) {
        ...
        if (weaponPacket.packet.schema) {
            obj.packet = h1z1_dataschema_1.default.parse(weaponPacket.packet.schema, weaponPacketData, weaponPacket.length, null).result;
        }
    }
    else {
        obj.packetType = weaponPacket.packetType;
...
function packWeaponPacket(obj) {
    const subObj = obj.packet, subName = obj.packetName, subType = weaponPacketTypes[subName];
    let data;
    if (weaponPacketDescriptors[subType]) {
        const subPacket = weaponPacketDescriptors[subType], subTypeData = writePacketType(subType);
        let subData = h1z1_dataschema_1.default.pack(subPacket.schema, subObj).data;
...
function parseItemData(data, offset, referenceData) {
    ...
    const baseItem = h1z1_dataschema_1.default.parse(itemBaseSchema, data, offset);
    ...
    detailItem = h1z1_dataschema_1.default.parse(detailSchema, data, offset);
...
function packItemData(obj, referenceData) {
    const baseData = h1z1_dataschema_1.default.pack(itemBaseSchema, obj.baseItem);
    ...
    detailData = h1z1_dataschema_1.default.pack(detailSchema, obj.detail);
...
const packets = [
            ...
            fields: [
                {
                    ...
                    fields: [
                        { name: "guid", type: "uint64", defaultValue: "" },
                        ...
                        { name: "creationDate", type: "uint64", defaultValue: "" },
                        { name: "unknownDword15", type: "uint32", defaultValue: 0 },
                        { name: "unknownDword16", type: "uint32", defaultValue: 0 },
                        { name: "unknownBoolean1", type: "boolean", defaultValue: false },
                        ...
                        { name: "unknownDword22", type: "uint32", defaultValue: 0 },
                        { name: "unknownDword23", type: "uint32", defaultValue: 0 },
                        { name: "unknownTime1", type: "uint64", defaultValue: "" },
                        { name: "unknownTime2", type: "uint64", defaultValue: "" },
                        ...
                        { name: "unknownQword1", type: "uint64", defaultValue: "" },
                        ...
                        { name: "unknownQword3", type: "uint64", defaultValue: "" },
                        {
    ...
    [
        ...
        {
            fields: [
                { name: "objectCharacterId", type: "uint64", defaultValue: "0" },
    ...
    [
        ...
        0x61,
        {
            fields: [
                ...
                { name: "unknownQword2", type: "uint64", defaultValue: "" },
                { name: "unknownFloat1", type: "float", defaultValue: 0.0 },
                { name: "unknownFloat2", type: "float", defaultValue: 0.0 },
                { name: "unknownFloat3", type: "float", defaultValue: 0.0 },
            ],
    ...
    [
        ...
        0x8506,
        {
            fn: function (data, offset) {
                const result = {}, startOffset = offset;
    ...
    [
        ...
        {
            fields: [
                { name: "ignore", type: "uint64", defaultValue: "0" },
                { name: "characterId", type: "uint64", defaultValue: "0" },
                {
                    name: "array1",
                    type: "array",
...
const [packetTypes, packetDescriptors] = packettable_1.default(packets);
